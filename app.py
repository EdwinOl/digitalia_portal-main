from dashboards import login, admin, virus, experiencia_2, mem, jj, artefacta, edwin, formu, formulario2, formulario3
import dash
...
def create_dashboard():
    ...
    app = dash.Dash(__name__,
                    external_stylesheets=[
                        dbc.themes.BOOTSTRAP, 'https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css',],
...
index_layout = html.Div([
                        html.Div(id='none', children=[], style={'display': 'none'}), html.Div(dcc.Store(id='user-info')), dbc.Row([html.H1("Elige tu ruta", id="title", style={"color": "#1b2b58", "font-size": "28px"})],
                                                                                                                                  justify='center', style={"margin-top": "2%", "margin-bottom": "2%"}), dbc.Col(dbc.Row(id="home-row-link", style={"min-width": "25%", 'display': 'flex'}), id='home-col-row-link')
...
# primera página jeje
edwin_layout = edwin.serve_layout()
edwin.init_callbacks(app)

# FORMULARIO 1
formu_layout = formu.serve_layout()
formu.init_callbacks(app)
# FORMULARIO 2
formulario2_layout = formulario2.serve_layout()
formulario2.init_callbacks(app)
# FORMULARIO 3
formulario3_layout = formulario3.serve_layout()
formulario3.init_callbacks(app)
...
def display_page(pathname):
    ...
    elif pathname.lower() == '/edwin':
        app.title = "Edwin"
        return edwin_layout
    elif pathname.lower() == '/formu':
        app.title = "Formulario"
        return formu_layout
    elif pathname.lower() == '/formulario2':
        app.title = "Formulario2"
        return formulario2_layout
    elif pathname.lower() == '/formulario3':
        app.title = "Formulario3"
        return formulario3_layout
...
def display_links(none):
    ...
    views = [
        {"name": "Admin", "path": "admin"}, {"name": "Virus", "path": "virus"}, {
            "name": "MEM", "path": "mem"}, {"name": "JJ", "path": "jj"}, {"name": "Edwin", "path": "edwin"}, {"name": "Formulario", "path": "formu"}, {"name": "Formulario2", "path": "formulario2"}, {"name": "Formulario3", "path": "formulario3"},
    ]

    links = [utils.link_format(view["name"], view["path"])
             for view in views if view["path"] in user["usuario_vistas"]]
    links = list(links)
    return links
